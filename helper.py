...
def log_game_results(agent, score, record, game, avg_loss=None):
    ...
    avg_reward = (
        total_reward / len(game.reward_history)
...
def check_metrics_alerts(metrics):
    ...
    if "efficiency_ratio" in metrics and metrics["efficiency_ratio"] is not None:
        ...
        if "efficiency_ratio" in ALERT_THRESHOLDS:
            if (
                "low" in ALERT_THRESHOLDS["efficiency_ratio"]
                and efficiency_val < ALERT_THRESHOLDS["efficiency_ratio"]["low"]
            ):
                alerts.append(
                    {
                        "level": "WARNING",
    ...
    if "w2_w1_ratio" in metrics and metrics["w2_w1_ratio"] is not None:
        ...
        if "weight_norm_ratio" in ALERT_THRESHOLDS:
            if (
                "critical" in ALERT_THRESHOLDS["weight_norm_ratio"]
                and ratio_val > ALERT_THRESHOLDS["weight_norm_ratio"]["critical"]
            ):
                alerts.append(
                    {
                        "level": "CRITICAL",
                ...
                "high" in ALERT_THRESHOLDS["weight_norm_ratio"]
                and ratio_val > ALERT_THRESHOLDS["weight_norm_ratio"]["high"]
            ):
                alerts.append(
                    {
                        "level": "WARNING",
